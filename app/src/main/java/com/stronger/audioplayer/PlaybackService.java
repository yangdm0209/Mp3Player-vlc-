...
public class PlaybackService extends Service implements IVLCVout.Callback {
    ...
    public void onCreate() {
        ...
        PowerManager pm = (PowerManager) AudioApplication.getAppContext().getSystemService(Context.POWER_SERVICE);
        ...
        filter.addAction(AudioApplication.SLEEP_INTENT);
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            TelephonyManager telManager = (TelephonyManager) AudioApplication.getAppContext().getSystemService(Context.TELEPHONY_SERVICE);
             ...
             * Sleep
             */
            if (action.equalsIgnoreCase(AudioApplication.SLEEP_INTENT)) {
                stop();
            }
        }
    };
...
//            VideoPlayerActivity.startOpened(AudioApplication.getAppContext(),
//                    getCurrentMediaWrapper().getUri(), mCurrentIndex);
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case SHOW_TOAST:
                    ...
                    Toast.makeText(AudioApplication.getAppContext(), text, Toast.LENGTH_SHORT).show();
                    break;
            }
        }
    }
    ...
    private void showNotification() {
        ...
        try {
            ...
            if (cover == null)
                cover = BitmapFactory.decodeResource(AudioApplication.getAppContext().getResources(), R.drawable.icon);
...
//            editor.putFloat(PreferencesActivity.VIDEO_SPEED, getRate());
    ...
    public Bitmap getCover() {
        if (hasCurrentMedia()) {
            return null; //AudioUtil.getCover(PlaybackService.this, getCurrentMedia(), 512);
        }
        return null;
    }
    ...
    public Bitmap getCoverPrev() {
        if (mPrevIndex != -1)
            return null; // AudioUtil.getCover(PlaybackService.this, mMediaList.getMedia(mPrevIndex), 64);
        else
            return null;
    }
    ...
    public Bitmap getCoverNext() {
        if (mNextIndex != -1)
            return null; // AudioUtil.getCover(PlaybackService.this, mMediaList.getMedia(mNextIndex), 64);
        else
            return null;
    }
    ...
    public void playIndex(int index, int flags) {
        ...
        if (mw.getType() != MediaWrapper.TYPE_VIDEO || mw.hasFlag(MediaWrapper.MEDIA_FORCE_AUDIO) || isVideoPlaying()) {
            ...
            notifyTrackChanged();
            determinePrevAndNextIndices();
            AudioApplication.runBackground(new Runnable() {
                @Override
                public void run() {
